...
export async function POST(req: Request) {
  ...
  const rate = checkRateLimit('admin-merch-thank-you', 30, 60_000)
  if (!rate.allowed) {
  ...
  try {
    ...
    await writeAdminAudit({
      ...
      entity: 'merch_order',
      entityId: id,
      payload: { type: 'merch', email: order.buyer_email },
    })

    return NextResponse.json({ ok: true, sent: true })
  } catch (error) {
    console.error('POST /api/admin/merch-orders/thank-you:', error)
    return NextResponse.json(
